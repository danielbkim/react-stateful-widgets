...
export default function Programmers() {
  ...
  programmerId ? style.color = 'gold' : style.color = 'royalblue'
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={ dev.id }>
              { dev.name } <button onClick={ () => { /* in here set the featured id to be dev.id */ setProgrammerId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          programmerId // this needs to be replaced by something 
